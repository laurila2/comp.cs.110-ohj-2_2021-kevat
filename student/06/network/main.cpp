...
void print(const Network &network,
...
int count(const Network &network,
          const std::string &current_id,
...
int depth(const Network &network,
          const std::string &current_id,
          int depth_tmp = 1)
{
    ...
    for (const std::string &child : network.at(current_id))
    {
        ...
        max_child_depth = std::max(current_child_depth, max_child_depth);
    }

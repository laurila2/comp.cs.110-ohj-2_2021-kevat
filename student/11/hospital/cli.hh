...
private:
    ...
    std::vector<Cmd> cmds_ = {
        ...
        {{"ENTER", "E"},
         ...
         &Hospital::enter},
        {{"LEAVE", "L"},
         "Take patient from hospital",
         {"patient id"},
         &Hospital::leave},
        {{"ASSIGN_STAFF", "AS"},
         "Assign staff for a patient",
         {"staff member id", "patient id"},
         &Hospital::assign_staff},
        {{"ADD_MEDICINE", "AM"},
         "Add medicine for a patient",
         {"medicine name", "strength", "dosage", "patient id"},
         &Hospital::add_medicine},
        {{"REMOVE_MEDICINE", "RM"},
         "Remove medicine from a patient",
         {"medicine name", "patient id"},
         &Hospital::remove_medicine},
        //{{"PRINT_PATIENT_MEDICINES", "PPM"},"Print patient's medicines",{"patient id"},&Hospital::print_patient_medicines},
        {{"PRINT_PATIENT_INFO", "PPI"},
         "Print patient's info",
         {"patient id"},
         &Hospital::print_patient_info},
        //{{"PRINT_PATIENTS", "PPS"},"Print patients per staff",{"staff member id"},&Hospital::print_patients_per_staff},
        {{"PRINT_CARE_PERIODS", "PCPS"},
         "Print care periods per staff",
         {"staff member id"},
         &Hospital::print_care_periods_per_staff},
        {{"PRINT_ALL_MEDICINES", "PAM"},
         "Print all used medicines",
         {},
         &Hospital::print_all_medicines},
        {{"PRINT_ALL_STAFF", "PAS"},
         "Print all staff",
         {},
         &Hospital::print_all_staff},
        {{"PRINT_ALL_PATIENTS", "PAP"},
         "Print all patients",
         {},
         &Hospital::print_all_patients},
        {{"PRINT_CURRENT_PATIENTS", "PCP"},
         "Print current patients",
         {},
         &Hospital::print_current_patients},
        {{"SET_DATE", "SD"},
         "Set date",
         {"day", "month", "year"},
         &Hospital::set_date},

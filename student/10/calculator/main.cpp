...
const vector<Command> COMMANDS = {{"+", 2, false, addition},
                                  {"-", 2, false, subtraction},
...
int main()
{
    ...
    while ( true ) {
        ...
        {
            command_to_be_executed[i] = static_cast<char>(
                toupper(command_to_be_executed[i]));
        }
        ...
        if (!(string_to_double(pieces.at(1), operand1)
              && string_to_double(pieces.at(2), operand2)))
        {
